...
That is all fine for most purposes, but the contract for which we have
prices is not traded on weekends, and also only during 8 to 22 o'clock
Europe/Berlin time.

<<fig=true>>=
plotTradingHours(x = prices,
                 t = times, interval = "1 sec", labels = "hour",
