...
uninterrupted up and down movements. 
...
rets <- 100*dax[up_down$end]/coredata(dax[up_down$start])

par(bty = "n", las = 1, tck = 0.01, 
    mar = c(3, 3, 1, 1), mgp = c(2, 0.5, 0), ps = 9)
plot(index(dax), rep(100, length(index(dax))), type = "l",
     lwd = 0.5, col = grey(.5), ylim = range(rets),
     xlab = "", main = "", log = "y", 
     xaxt = "n", yaxt = "n",
     ylab = "Streak in %")
...
abline(h = 100, lwd = .5, col = grey(.5))
abline(h = axTicks(2), col = grey(0.7), lwd = 0.25)
abline(h = 10^par("usr")[3])
