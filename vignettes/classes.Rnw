...
\begin{itemize}
\item store a 'portfolio' of assets, evaluate for new market data;
  possibility to change correlation assumptions
\item reevaluate for scenario (= new market data or artificial data)
\item read in transactions from various sources
...
\item Instrument: term sheet (description etc); it does know nothing
  about market data
\item Position: only for printing
