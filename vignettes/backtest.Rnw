...
asset. To test this strategy, we call \texttt{backtest}. We
define initial cash \texttt{c0} of 100. (This is also the
default.)
<<>>=
...
We see that we bought in the second period because the default
setting for the burnin \texttt{b} is 1; thus, we lose one
observation.

Since we do not rely in any way on the past in this trading rule,
we set \texttt{b} zo zero.
