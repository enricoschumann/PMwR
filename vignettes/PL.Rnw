...
We buy one unit of an asset at a price of 100; we sell one unit at
102.
...
For situations like this, the function \texttt{plsorted} can be used.

<<>>=
plsorted(price  = c( 90, 50, 100), 
         amount = c(  1,  1,  -2))
@ 
...
\PMwR\ provides a simple, implictly-defined class \texttt{Journal}
that should help to handle trades.  A \texttt{Journal} is created
...
Journal(timestamp = trades$timestamp, 
