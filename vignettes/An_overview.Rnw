...
\item \texttt{journal}: keeps transactions. Internally, a object of
  class journal is named list of atomic vector.
\item \texttt{position}: comprises the numerical positions of
  different accounts/instruments, the timestamps and the description
  of the instruments. Internally, it is always a numeric matrix with
  attributes timestamp and instrument; points in time are in rows,
  instruments in columns.
