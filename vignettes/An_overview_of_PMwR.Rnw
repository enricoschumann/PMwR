...
tools that help with processing and analysing trade and
...
\Sexpr{pv}, \Sexpr{nt} tests are included.
These tests are stored in subdirectory \texttt{tinytest}.} %
...
The very latest version of the package is available from
