...
\noindent \textsc{pm}w\textsc{r} provides a small set of
reliable, efficient and convenient tools that help with
processing and analysing trade and portfolio data. The
package does not provide a complete application to be used
`as is', but tools for creating such an application.
...
The interfaces of functions may still not be fully stable
(e.g., argument names might still be made consistent across
functions); in some cases, generic functions might be
introduced.%
\footnote{
<<echo=false,results=hide>>=
if (requireNamespace("tinytest")) {
    ...
    nt <- "several hundred"
@
Even if \textsc{pm}w\textsc{r} is under development: the
package is to provide well-documented and reliable code.
For all computations, unit tests are included.  As of
package version %
\Sexpr{pv}, \Sexpr{nt} tests are included.  These tests are
stored in subdirectory \texttt{tinytest}.} %
The \textbf{recommended practice} is therefore to
\textbf{explicitly name arguments in function calls} and
not pass arguments by position. Any changes in argument
names will be documented in the \texttt{NEWS} file
(\,\url{http://enricoschumann.net/R/packages/PMwR/NEWS}\,)
and so can be easily followed. More details are in the
ChangeLog
...
from \url{http://enricoschumann.net/R/packages/PMwR/}\,. To
...
% latest version.%
% \footnote{Of course, \textsc{cran} provides Windows
%  binaries for its version.} %
% If you wish to use the latest version of the package on
...
% \noindent New package releases are announced on the
%  \texttt{PMwR-news} mailing list:\smallskip

% \url{https://lists.r-forge.r-project.org/cgi-bin/mailman/listinfo/nmof-pmwr}
% \medskip

% \noindent An \textsc{rss} feed of the package \textsc{news} file is
% available at\smallskip

% \url{http://enricoschumann.net/R/packages/PMwR/PMwR_news.xml}\,.
