...
application that could be used `as is', but building
blocks for creating such an application.
...
\noindent The manual is a draft and it will change as
frequently as the package. I am grateful for comments,
