...
\footnote{In the unlikely case that you come across a really old
  version of the package: it was called \textsc{pm} bevore 2012.} %
...
need of being groomed for general use.%
\footnote{
<<echo=false>>=
...
\item[Computing \textsc{p}\&\textsc{l} and returns] See
  \texttt{?returns}, \texttt{?pl} and \texttt{?unit\_prices}.
\end{description}
