...
We have an account; currency is euro. We buy one unit of an asset
at a price of 100 euro, and then we sell the asset for 102
euro. We have made a profit of 2 euros. This simple case is
frequent enough that we should make the required computation
simple as well. 

With the PMwR package, computing profit-or-loss (P/L) can be
handled through the function =pl=. And for the simple case above,
it may be called like as follows.

@@latex:\index{pl@\texttt{pl} (function)}@@
...
Another example. Suppose that a trader bought one unit of an
asset at 50, one unit at 90 and sold two units at 100, resulting
in a profit of 60.
#+BEGIN_SRC R -n :exports both :results output
...
That looks like some pretty good trading. Yet suppose that the
...
information in the order of the trades and the drawdown that it
...
price. This may not be totally accurate because of bid--ask
spreads or other transaction costs. But it provides more
information than only computing the aggregate P/L for the trades.

## TODO EURUSD trades (many)
...
The warning message that is thrown already tells us what to do:
to close the trade -- only for the purpose of computing P/L, the
trade is not really closed --, specify =eval.price=.
#+BEGIN_SRC R -n :exports both :results output
...
Note that /average sell/ reflects the specified evaluation price;
but /volume/ does nothing since nothing is actually traded.
...
income and expenses. An aside: it is tedious to enter journals by
...
One way is to switch to double-entry accounting. A second way is
to add descriptions like =expense= and then subset the journal by
these descriptions.
...
120.2, it actually means `120 euros and 20 cents, or 120.20
euros'. The price of one euro -- no surprise here -- is one euro.

There are just two rules to remember: 1) An expense should be a
negative amount; income should go with positive amounts.  2) When
an entry means gain or loss to us, we make its price 0. Then,
when we compute that value of a position, we make its price 1.
An example:
...
which is exactly what the function =pl= reports.
#+BEGIN_SRC R -n :exports both :results output
...
The function =scale1= rescales time-series. It is a generic
function; currently there are methods for numeric
vectors/matrices and for =zoo= objects.
...
#+RESULTS:
:  
: [1] 1.0000000 1.0037351 1.0029745 1.0112936 1.0104042 1.0022947 1.0077872
:  [8] 1.0117159 1.0149436 1.0355073 1.0337945 1.0364807 1.0336020 1.0351147
: [15] 1.0340499 1.0245744 0.9991468 0.9945936 1.0007308 0.9932649 0.9971745
: [22] 0.9900469

With =centre=, the daily mean /return/ is subtracted.
#+BEGIN_SRC R -n :exports both :results output
scale1(x, centre = TRUE)
#+END_SRC 

#+RESULTS:
:  [1] 1.000000 1.004213 1.003930 1.012740 1.012331 1.004685 1.010672 1.015095
:  [9] 1.018819 1.039956 1.038731 1.041926 1.039527 1.041544 1.040969 1.031921
: [17] 1.006791 1.002680 1.009348 1.002295 1.006720 1.000000

With =scale=, the daily /returns/ are rescaled to to that
standard deviation.
#+BEGIN_SRC R -n :exports both
...
I stress that /returns/ are treated, but =scale1= expects price
series (i.e. levels, not returns) and also returns a price
series.
