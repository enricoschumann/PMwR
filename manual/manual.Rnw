...
\begin{document}
{\raggedright{\Large The PMwR Manual}}\medskip
...
of transactions which, essentially, are nothing more than dataframes.
...
journal should always be retrieved by name, never by position (which
is different for a dataframe, for which we can meaningfully speak of
the second column, say).
...
What is actually stored in a \texttt{journal} is up to the analyst,
but a few fields are typical (and required for several methods):
...
then there are many tools to transform.
...
Suppose some of these transactions mean actually a gain or loss to us
(a dividend payment, say), while others are neutral (such as a
transfer between accounts).  How can we deal with that?  One way is to
...
one.  That is reasonable: if I my account has a balance of, say,
`120.2', that it actually means `120.2 euros'.  When evaluated in
euro, the price is one.  

An expense should be a negative amount; income should go woth positive
amounts.  When you add such an entry, make its price 0.



<<>>=
jnl <- journal(timestamp = 1:3, 
...
\noindent Thus, we have had a position from time zero to 12 (hours,
say), but its size varied.  The function \texttt{twExposure}
(time-weighted exposure) computes the average absolute exposure.
