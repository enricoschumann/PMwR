...
A~\texttt{journal} is created through the function
\texttt{journal};\index{journal!vs data.frame} it is a list of atomic
vectors with a class attribute.  Methods should not rely on this list
being sorted in any particular way: components of a journal should
always be retrieved by name, never by position (which is different for
a dataframe, for which we can meaningfully speak of the \emph{n}th
column).  I will refer to these journal components, such as
\texttt{amount} or \texttt{timestamp}, as
fields.\index{journal!fields}
...
\texttt{account} and \texttt{instrument}; \ldots
<<>>=
jnl2 <- jnl
jnl2$instrument <- paste0(jnl$account, ":", jnl$instrument)
position(jnl2)
@ 
\ldots the other is to use \texttt{split} on the journal.
<<>>=
...
We may also want to compute the profit/loss between two points in
...
You can also use this mechanism if you wish to compute the profit/loss
...
For a single instrument, computing profit/loss in units of the
instrument is always meaningful (though sometimes, perhaps, not always
intuitive).  \emph{Adding up} the profits and losses of several assets
often does not work because of multipliers or different currencies.

The  simplest and most transparent way is to manipulate the journal
before pl is computed (eg, multiply 

The function \texttt{pl} has an argument \texttt{multiplier}.
...
goal: to compute the end-of-day pl 
