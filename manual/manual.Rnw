...
transactions.  \texttt{PMwR} provides an S3 class \texttt{Journal} for
...
\item[{\ldots{}}] other fields. They must be named, for instance \texttt{fees =
          c(1,2,1)}
\end{description}
...
\texttt{<account>::<instrument>}.  But there is nothing enforce it.
